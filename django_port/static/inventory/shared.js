...
export function normalizeRawScan(raw) {
    return (raw ?? "").replace(/[\r\n\t]+/g, " ").trim();
}
...
export function setBadgeState(badge, state, text) {
    if (!badge) {
        return;
    }
    const nextClass = state === "detectando"
...
export function mountScannerWorkflow(options) {
    ...
    const submitCode = (raw, reason = "submit") => {
        ...
        input.value = code;
        track?.("web.scan_submit_requested", "La pagina va a enviar un tag escaneado.", { uid: code, reason }, "info", "web_ui");
    ...
    const flushBuffer = (reason = "flush") => {
        ...
        if (value.length > 0) {
            ...
            track?.("web.scan_short_frame", "Se detecto una trama demasiado corta para ser UID valido.", { raw_value: value, reason }, "warning", "web_ui");
        }
    };
    const scheduleFlush = () => {
        if (flushTimer !== null) {
    ...
    input.addEventListener("input", () => {
        ...
        track?.("web.scan_input_detected", "El input oculto recibio caracteres del lector.", { raw_value: currentValue, method: "input" }, "info", "web_ui");
        scheduleFlush();
    });
    document.addEventListener("paste", (event) => {
        ...
        track?.("web.scan_input_detected", "La pagina detecto una lectura por pegado rapido.", { raw_value: pasted, method: "paste" }, "info", "web_ui");
        scheduleFlush();
    }, true);
